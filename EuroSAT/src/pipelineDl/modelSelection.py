# modelSelection.py — EuroSAT Deep Learning Model Selection (Optuna)
import os
import sys
import argparse
import json
...
def prepare_dataset(root_path: str, split_ratio: float):
    ...
    optuna_size = int(len(train_dataset) * 0.3)
...
def objective(trial, data, augmented_data, config) -> float:
    ...
    """
    train_dataset = data
    ds = config.get("dataset_name", "eurosat")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="EuroSAT DL Model Selection (Optuna)")
    ...
    if not os.path.isabs(project_cfg_initial['cache_directory']):
        project_cfg_initial['cache_directory'] = os.path.join(project_root, project_cfg_initial['cache_directory'])
    ...
    # --- Dataset ---
    ds_train, ds_test, ds_optuna = prepare_dataset(
    ...
    for run_idx in range(1, NUM_RUNS + 1):
        ...
        optuna_run_metrics = utils.run_optimization(
            objective, ds_optuna, None, project_cfg, optuna_cfg
        )
